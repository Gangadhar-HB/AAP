...
save_configuration ()
{
   ...
   # Save cosign policy
    ...
    if [ -f "/usr/local/bin/backup_mdatp.sh" ]; then
        disp_msg $INFO "Executing backup_mdatp.sh..."
        /bin/bash /usr/local/bin/backup_mdatp.sh
        if [[ $? -ne 0 ]]; then
...
restore_configuration ()
{
   ...
   # Restore sentinelone file
    ...
    if [ -f "/usr/local/bin/restore_mdatp.sh" ]; then
        disp_msg $INFO "Executing restore_mdatp.sh..."
        /bin/bash /usr/local/bin/restore_mdatp.sh
        if [[ $? -ne 0 ]]; then
